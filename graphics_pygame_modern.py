...
class Graphics:
    ...
    COLOUR_BACKGROUND = pygame.Color("Gray 15")
    COLOUR_COORDS = pygame.Color("Gray 80")
    COLOUR_OUTLINE = pygame.Color("Black")
    ...
    def __init__(self):
        ...
        self.font_info_title = pygame.font.SysFont("palatinolinotype", size=self.FONT_SIZE_AUTHOR)
        self.font_info_title_bold = pygame.font.SysFont("palatinolinotype", size=self.FONT_SIZE_INFO)
        self.font_info_title_bold.set_bold(True)
    ...
    def _draw_board_coords(self):
        for i in range(c.PLAYFIELD_SIZE):
            ...
            self._draw_text(int_print, (x, y), self.COLOUR_COORDS, self.COLOUR_BACKGROUND, self.font_coords)
            ...
            self._draw_text(char_print, (x, y), self.COLOUR_COORDS, self.COLOUR_BACKGROUND, self.font_coords)
    ...
    def draw(self, game: Game):
        self._draw_top_bar(game.level_info)
        self._draw_sidebar()
        ...
        pygame.draw.rect(self.screen, self.COLOUR_OUTLINE, self.rect_playfield, width=2)
        for sq in SQUARES:
    ...
    def _draw_top_bar(self, info: engine.LevelInfo):
        pygame.draw.rect(self.screen, self.COLOUR_BACKGROUND, self.rect_menu, width=0, border_radius=0)

        rendered_info = self.font_info_title_bold.render(info.title, True, self.COLOUR_COORDS, self.COLOUR_BACKGROUND)
        rendered_info_rect = rendered_info.get_rect(midleft=(self.INFO_PADDING, int(self.MENUBAR_SIZE / 2)))
        self.screen.blit(rendered_info, rendered_info_rect)

        rendered_author = self.font_info_title.render(" by " + info.author, True, self.COLOUR_COORDS,
                                                      self.COLOUR_BACKGROUND)
        rendered_author_rect = rendered_author.get_rect(midleft=(rendered_info_rect.right, int(self.MENUBAR_SIZE / 2)))
        self.screen.blit(rendered_author, rendered_author_rect)
    ...
    def _draw_sidebar(self):
        ...
        pygame.draw.rect(self.screen, self.COLOUR_BACKGROUND, sidebar_rect, width=0, border_radius=0)
